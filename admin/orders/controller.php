...
function doInsert(){
	if(isset($_POST['save'])){
		...
		if ( $errofile > 0) {
			...
			if ($image_size==FALSE || $type=='video/wmv') {
				message("Загруженный файл не является изображением!", "error");
				redirect("index.php?view=add");
			}else{
					//uploading the file
					...
					$messageStats = false;
					message("Все поля обязательны для заполнения!","error");
					redirect('index.php?view=add');
					}else{	
						...
						if ($res >=1) {
							message("Название продукта уже существует!", "error");
							redirect("index.php?view=add");
						}else{
	...
	function doEdit(){
		...
		if ($_GET['actions']=='confirm') {
				...
				$remarks ='Ваш заказ подтвержден.';
				$delivered = Date('Y-m-d');

		}elseif ($_GET['actions']=='deliver') {
							# code...
				$status	= 'Доставлен';	
				$remarks ='Ваш заказ был доставлен.';
				$delivered = Date('Y-m-d');
				 
		}elseif ($_GET['actions']=='cancel'){
			// $order = New Order();
				$status	= 'Отмененный';
				$remarks ='Ваш заказ был отменен из-за отсутствия связи и неполной информации.';
		}
  	...
  	// 		$res = $customer->single_customer($_GET['customerid']); 
	        ...
	        $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$cur->PHONE."','','От E-builder shop  : Ваш заказ был '".$status. "'. Цена '".$cur->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
			...
			foreach ($cur as $result) {
	  		 ...
	  		 $sql = "INSERT INTO `messageout` (`Id`, `MessageTo`, `MessageFrom`, `MessageText`) VALUES (Null, '".$result->OWNERPHONE."','','От E-builder shop  : Ваш товар был закаан'. Цена '".$result->PAYMENT. "')";
	        $mydb->setQuery($sql);
	        $mydb->executeQuery();
	...
	if (isset($_POST['selector'])==''){
		message("Сначала выберите записи, прежде чем удалять!","info");
		redirect('index.php');
	}else{
   ...
   if(isset($_GET['id'])){
    ...
    $price= $_GET['price'];
      ...
      addtocart($pid,1,$price);
			...
			message("в корзину добавлен 1 товар", "success");
			redirect("index.php?view=add");
    ...
    for($i=0;$i<$max;$i++){
      ...
      if($qty>0 && $qty<=9999){
        ...
        $_SESSION['fixnmix_cart'][$i]['price']=$price;
			...
			message("Корзина была обновлена.", "success");
			redirect("index.php?view=add");
	...
	function cartDelete(){
		...
		message("1 товар был удален из корзины.");
	...
	function processorder(){
 		...
 		// 	unset($_SESSION['alltot']);
			message("Новый заказ создан успешно!", "success"); 		 
