...
function doInsert(){
	if(isset($_POST['save'])){
		...
		if ( $errofile > 0) {
				message("Изображение не выбрано!", "error");
			...
			if ($image_size==FALSE || $type=='video/wmv') {
				...
				redirect("index.php?view=add");
					...
					redirect('index.php?view=add');
						...
						message("Новый продукт создан успешно!", "success");
						redirect("index.php");
						}
	...
	function doEdit(){
		...
		if(isset($_POST['save'])){
			...
			message("Продукт был обновлен!", "success");
			redirect("index.php");
	  }
	redirect("index.php"); 
}
	...
	function doDelete(){
		...
		if (isset($_POST['selector'])==''){
			message("Сначала выберите записи, прежде чем удалять!","error");
			redirect('index.php');
			}else{
	...
	function doupdateimage(){
		...
		if ( $errofile > 0) {
			...
			if ($image_size==FALSE ) {
				message("Загруженный файл - это не изображение!
				", "error");
				redirect("index.php?view=view&id=". $_POST['proid']);
			}else{
					//uploading the file
