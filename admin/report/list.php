 	    ...
 	    <div class="row">
		  <div class="col-md-12">
			 <div class="form-group input-group" style="margin-top:25px;">  
                <button class="btn btn-primary btn-sm" name="submit" type="submit" >Поиск <i class="fa fa-search"></i>
                </button> 
            </div>
		   </div>  
	    </div> 
...
<span id="printout">
	<div class="col-md-12" >
	<div class="page-header" style="text-align:center;" ><h1>Список заказанных товаров</h1>
		<div>Даты включительно: От : <?php echo isset($_POST['date_pickerfrom']) ? $_POST['date_pickerfrom'] :'';?> - до : <?php echo isset($_POST['date_pickerto']) ? $_POST['date_pickerto'] : '';?> </div>
	</div>
...
//            date(ORDEREDDATE) <= '". date_format(date_create($_POST['date_pickerto']), "Y-m-d")."'";
			...
			$totQty +=$result->QTY;				 
 			$totAmount += $AMOUNT;
									...
									echo '<tr><td colspan="7" align="center"><h2>Пожалуйста, Введите Даты</h2></td></tr>';
								}
...
<tfoot>
	...
	<div class="row">
		<div class="col-md-12">
			<div class="col-md-2"> 	
			<button onclick="tablePrint();" class="btn btn-primary"><i class="fa fa-print"></i> Распечатать отчет</button>
 		</div>
	  </div>
	</div>
</div>
