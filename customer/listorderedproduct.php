  ...
  <div class="modal-content">
	<div class="modal-header">
		...
		<table>
			<tr>
				<td align="center"> 
				<img src="<?php echo web_root; ?>images/home/lbuild.png"   alt="Image">
        		</td> 
			</tr>
		</table>
...
?>
 	 <h4>Ваш заказ подтвержден и готов к отправке</h4><br/>
		<h5>Уважаемый ,</h5>
		<h5>Поскольку вы заказали оплату наличными при получении, пожалуйста, укажите точную сумму наличными для оплаты нашим сотрудникам и предоставьте данные для выставления счета.</h5>
		 <hr/>
		 <h4><strong>Инфо о доставке</strong></h4>
		 <div class="row">
		 	<!-- <div class="col-md-6">
		 		<p> ORDER NUMBER : <?php echo $_SESSION['ordernumber']; ?></p>
		 		<?php 
		 			$query="SELECT UZS(ORDEREDQTY) as 'countitem' FROM `tblorder` WHERE `ORDEREDNUM`='".$_SESSION['ordernumber']."'";
		 			$mydb->setQuery($query);
...
<?php 
}elseif ($cur->PAYMENTMETHOD=="Оплата при доставке"){
		 
?>
 	 <h4>Ваш заказ подтвержден и доставлен</h4><br/>
 		<h5>Уважаемый ,</h5>
		<h5>Ваш заказ уже в пути! Поскольку вы сделали заказ наложенным платежом, пожалуйста, укажите точную сумму наличными нашему поставщику.	</h5>
		 <hr/>
		 <h4><strong> Инфо о доставке</strong></h4>
		 <div class="row">
		 	<div class="col-md-6">
		 		<p> Номер заказа : <?php echo $_SESSION['ordernumber']; ?></p>
					...
					$res = $mydb->loadResultList();
					?>
		 		<p>Товары, подлежащие доставке : <?php
		 		foreach ( $res as $row) echo $row->countitem; ?></p> 
...
<?php 
	 ...
	 echo "Ваш заказ был отменен из-за отсутствия связи и неполной информации.";

}else{
	echo "<h5>Ваш заказ находится в процессе обработки. Пожалуйста, проверьте свой профиль на наличие уведомления о подтверждении.</h5>";
} 
?>
<hr/>
 <h4><strong> Инфо о заказе</strong></h4>
				 ...
				 $subtot=0;
				  $query = "SELECT * 
						...
						echo '<tr>';  
				  		...
				  		echo '<td> UZS '. number_format($result->PROPRICE,2).' </td>';
				  		echo '<td align="center" >'. $result->ORDEREDQTY.'</td>';
				  		?>
				  		 <td> UZS <output><?php echo  number_format($result->ORDEREDPRICE,2); ?></output></td> 
				  		<?php
		...
		?>
 		...
 		<div class="row">
		  	<div class="col-md-6 pull-left">
		  	 <div>Заказанная дата: <?php echo date_format(date_create($cur->ORDEREDDATE),"M/d/Y h:i:s"); ?></div> 
		  		<div>Тип оплаты : <?php echo $cur->PAYMENTMETHOD; ?></div>

		  	</div>
		  	<div class="col-md-6 pull-right">
		  		<p align="right"> Итоговая цена: UZS <?php echo number_format($subtot,2);?></p>
		  ...
		  ?>
		   <hr/> 
		  <div class="row">
		 		 <p>Пожалуйста, распечатайте это в качестве подтверждения покупки</p><br/>
		  	  <p>Мы надеемся, что вам понравятся приобретенные вами товары. Хорошего дня!</p>
		  	  <p>С Уважением.</p>
		  	  <h4><a href="">Спаисбо</a></h4>
		  </div>
		  <?php }?>
  </div> 
		...
		<?php if($cur->ORDEREDSTATS!='Pending' || $cur->ORDEREDSTATS!='Cancelled' ){ ?> 
                ...
                <button  onclick="tablePrint();" class="btn btn_fixnmix pull-right "><span class="glyphicon glyphicon-print" ></span> Печатать</button>     
