...
function doInsert(){
	...
	if(isset($_POST['submit'])){
			 ...
			 if(!isset($_POST['proid']) || (isset($_POST['proid']) && empty($_POST['proid']))){
			  echo "<script> alert('You are now successfully registered. It will redirect to your order details.'); </script>";
			 	...
			 	$mydb->setQuery($query);
			 	$mydb->executeQuery();
			 	 echo "<script> alert('Теперь вы успешно зарегистрированы. Он будет перенаправлен на ваш профиль.'); </script>";
	...
	function doEdit(){
		if(isset($_POST['save'])){
			...
			message("Учетные записи были обновлены!", "success");
			redirect(web_root.'index.php?q=profile');
		}
	}
	...
	function doDelete(){
		...
		if (isset($_POST['selector'])==''){
			...
			message("Клиент был удален!","info");
			redirect('index.php');
